...
search_data = (2017, 1.6, 36000)
"""unpack search criteria tuple"""
year, engine, price = search_data

"""filter cars by criteria"""
filt = {(name, data) for name, data in car_data.items()
        if data[1] >= year and data[2] >= engine and data[4] <= price
        }
"""sorted cars by price"""
sorted_cars = sorted(filt, key=lambda x: x[1][4])

"""print 5 first found elements"""
for key, value in sorted_cars[:5]:
    print(f'{key}: {value}')
